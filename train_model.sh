...
python3 model.py --model . --data data/ --epochs 150 2>&1 | tee train_model.txt && \
  ...
  python3 model.py --model . --data car32/ --epochs 150 --load model.json 2>&1 | tee car32.txt && \
  git add car3*.txt && \
  cp weights.hdf5 model.h5 && \
  git commit -a -m "trained new model with car3 data" && git push \ 
  python3 model.py --model . --data car_3/ --epochs 150 --load model.json 2>&1 | tee car_3.txt && \
  python3 model.py --model . --data car_2/ --epochs 150 --load model.json 2>&1 | tee car_2.txt && \
